...
THREE.AlphaVideoTextureUtil = {
  
  supportsWebMAlpha: function( callback ) {
    
    var IE = navigator.userAgent.indexOf( 'MSIE ' ) > -1,
        vid = document.createElement('video'),
        canvas;
    
    if ( !vid.canPlayType( 'video/webm' ) ) callback( false );
    
    vid.autoplay = false;
    vid.loop = false;
    if ( IE ) vid.style.display = 'none';
    
    vid.addEventListener( 'loadeddata', function() {
        
      if ( IE ) document.body.removeChild(vid);
      ...
      if ( ctx.getImageData( 0, 0, 1, 1 ).data[3] === 0 ) callback( true );
      else callback( false );
    ...
    vid.addEventListener( 'error', remove );
    vid.addEventListener( 'stalled', remove );
    vid.addEventListener( 'abort', remove );
    ...
    if ( IE ) document.body.appendChild(vid);
...
THREE.AlphaVideoTexture = function( src, options, mapping, wrapS, wrapT, magFilter, minFilter, format, type, anisotropy ) {
  ...
  this.processPixels = function() {
    ...
    if ( lastDrawnFrameTime !== currentFrameTime && this.video.readyState > 1 && !renderCount ) {
      ...
      if ( this.imageTexture == THREE.AlphaVideoTextureImageTexture.DATA ) {
  ...
  function init( supported ) {
    ...
    if ( !supported ) {
      ...
      if ( src.webm ) sources += '<source src="'+src.webm+'" type="video/webm">';
  ...
  THREE.AlphaVideoTextureUtil.supportsWebMAlpha( init.bind( this ) );
